...
const modifyFiles = [
  ...
  'package.json',
  'README.md',
  'Tests/HelloTests/HelloTests.swift',
  'Tests/HelloTests/XCTestManifests.swift',
  'Tests/LinuxMain.swift',
  '.vscode/launch.json'
];
const renameFiles = [
  ['Sources/Hello', 'Sources/--ProjName--'],
  ['Tests/HelloTests/HelloTests.swift', 'Tests/HelloTests/--ProjName--Tests.swift'],
  ['Tests/HelloTests', 'Tests/--ProjName--Tests']
];

const _promptSchemaLibraryName = {
  properties: {
    library: {
      description: colors.cyan('What do you want the project to be called? (use PascalCase)'),
      pattern: /^([A-Z][a-z]+)*[A-Z]([a-z]|\d)*$/,
      type: 'string',
      required: true,
      message: '"PascalCase" uses Uppercase letters, and without hyphens for any punctuation'
    }
  }
};
...
// Say hi!
console.log(colors.cyan("Hi! You're almost ready to make the next great Swift project."));
...
function modifyContents(libraryName: string, username: string, usermail: string) {
  ...
  try {
    const changes = replace.sync({
      files,
      from: [/--ProjName--/g, /--username--/g, /--usermail--/g],
...
function renameItems(libraryName: string) {
  ...
  renameFiles.forEach(function(files) {
    ...
    let newFilename = files[1].replace(/--ProjName--/g, libraryName);
